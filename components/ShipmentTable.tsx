...
export default function QuoteRequestShipments() {
    ...
    const fetchShipments = async () => {
      ...
      try {
        ...
        const sortedShipments = (data.details || []).sort(
          (a: Shipment, b: Shipment) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime(),
        )
  ...
  const getTotalWeight = (details: ShipmentDetail[]) => {
    ...
    <div className="bg-white">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-800">List of all requested shipments (Latest first)</p>
        </div>
      </div>
