...
export default function UserDashboard() {
  ...
  const [shipmentsLoading, setShipmentsLoading] = useState(true) // Changed back to true

  // Optimized shipments fetching
  useEffect(() => {
    if (!user?.id) {
    ...
    const fetchUserShipments = async () => {
      try {
        setShipmentsLoading(true) // Ensure loading is set
        ...
        if (response.ok) {
          ...
          if (data && data.details) {
            const sortedShipments = data.details
              .sort(
                (a: UserShipment, b: UserShipment) =>
        ...
        setShipmentsLoading(false) // Always stop loading
      }
    }
  ...
  // Show content immediately, only show loading for specific sections
  return (
    <div className="bg-white">
      ...
      {/* Quick Summary Section - Show immediately */}
